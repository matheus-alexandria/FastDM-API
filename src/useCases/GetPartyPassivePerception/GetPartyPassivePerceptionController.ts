...
class GetPartyPassivePerceptionController {
  handle(request: Request, response: Response) {
    ...
    const getPartyPassivePerceptionUseCase = new GetPartyPassivePerceptionUseCase(
      prismaCharactersRepository
    );

    const passives = getPartyPassivePerceptionUseCase.execute(
      String(method),
      Number(reference)
    );
